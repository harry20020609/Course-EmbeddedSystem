...
#define DISIRED_DISTANCE_LEFT 1.2
...
public:
    ...
    void pid_control() {
        ...
        double new_error = 0.9 * error + 0.1 * prev_error;
        if(error>=DISIRED_DISTANCE_LEFT) return ;
        double angle = -(KP * new_error + KD * (error - prev_error) / del_time + KI * integral);
        if(angle<minAngle){
		    ackermann_drive_result.drive.steering_angle = minAngle;
	    }
	    else if(angle>maxAngle){
		    ackermann_drive_result.drive.steering_angle = maxAngle;
        }
        else{
